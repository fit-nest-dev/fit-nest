...
const AddTrainerDialog = ({ fetchTrainers, openDialog, setOpenDialog }) => {
      ...
      try {
        ...
        for (let i = 0; i < excelData.length; i++) {
          ...
          addTrainerPromises.push(
            axios.post(
              'http://3.25.86.182:5000/api/Trainer/AddTrainer',
  ...
  const handleAddTrainersManual = async () => {
    if (window.confirm('Are you sure you want to add this trainer?')) {
      try {
        const response = await axios.post('http://3.25.86.182:5000/api/Trainer/AddTrainer', TrainerData, { withCredentials: true });
        if (response.data) {
          fetchTrainers();
        }
